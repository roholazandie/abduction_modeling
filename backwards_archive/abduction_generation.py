...
def forward_sample(model,
    ...
    while cur_len < max_length:
        ...
        if i < model_config.length:
            ...
            #backward_logits[:, :, :]
            #res = torch.max(backward_logits, dim=1)[0]
            #perturbed_next_token_logits = mix_rate * next_token_logits + (1 - mix_rate) * res
